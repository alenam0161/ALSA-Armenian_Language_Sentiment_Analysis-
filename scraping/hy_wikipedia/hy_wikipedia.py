...
while(True):
    ...
    wikidat = itertools.chain.from_iterable([i.get_text().split('։') for i in soup.find_all('p') if i.get_text()])  

    #write to file each page
    with open("test22.cor", "a") as f:
        for i in wikidat:
            f.write('%s\n'%i.replace('\n',''))
    
    #sleep 0.5 seconds not to cause denial of service
    time.sleep(0.5)